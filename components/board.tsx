...
export function KanbanBoard({initialTasks = [], initialColumns = []}: KanbanBoardProps) {
  ...
  function getDraggingTaskData(taskId: UniqueIdentifier, column_id: number | null) {
    const tasksInColumn = tasks.filter((task) => task.column_id === column_id);
  ...
  const announcements: Announcements = {
    onDragStart({ active }) {
      ...
      if (active.data.current?.type === "Column") {
        const startcolumn_idx = columnsId.findIndex((id) => id === active.id);
        const startColumn = columns[startcolumn_idx];
    ...
    onDragOver({ active, over }) {
      ...
      if (
        ...
        const overcolumn_idx = columnsId.findIndex((id) => id === over.id);
        return `Column ${active.data.current.column.name} was moved over ${
          over.data.current.column.name
        } at position ${overcolumn_idx + 1} of ${columnsId.length}`;
      } else if (
